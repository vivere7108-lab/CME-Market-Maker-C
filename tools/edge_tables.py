...
S = "/tmp/claude-1000/-home-bread/c6f0bedf-f019-49ee-a3b4-61a9bece47fd/scratchpad"
SPECS = json.load(open(S + "/specs.json"))
ORDER = ["ES", "NQ", "CL", "GC", "ZN", "6E", "ZC", "SR3"]
...
def main():
    products = [load(p, S + "/events/B_%s_*.csv" % p) for p in ORDER]
    ...
    for behind, label, qa in ((1, "B.  ONE TICK BEHIND THE TOUCH  (what the strategy actually quotes)", True),
        ...
        print(label)
        if behind == 1:
            print("    A fill here is always a sweep that went through the level in front, so there is no")
            print("    queue assumption in it. 'net' is the 60s markout less two fees: in and out.")
        else:
    ...
    print()
    print("=" * 108)
    print("D.  INFORMATIONAL EDGE  (one tick behind, warmed-up gate, 60s markout)")
    ...
    print("    as a classifier of loss-making fills; 0.50 is no information.")
    print("=" * 108)
    t = table(products, 1)
    w = [5, 8, 8, 8, 8, 8, 8, 8, 10, 10]
    print(row(["prod", "n warm", "IC all", "IC ofi", "IC depl", "IC run", "IC vpin", "AUC", "worst 10%", "best 10%"], w))
    print("-" * 108)
    for r in t:
        if not r.get("n") or r.get("n_warm", 0) < 200:
            print(row([r["product"], str(r.get("n_warm", 0)), "too few", "", "", "", "", "", "", ""], w))
            continue
        print(row([r["product"], "%d" % r["n_warm"], "%+.3f" % r["ic"], "%+.3f" % r["ic_ofi"], "%+.3f" % r["ic_depl"],
                   "%+.3f" % r["ic_run"], "%+.3f" % r["ic_vpin"], "%.3f" % r["auc"],
                   "%+.2f" % r["worst_decile_usd"], "%+.2f" % r["best_decile_usd"]], w))
    return products
...
def curves(products):
    print()
    print("=" * 108)
    print("E.  THE MARKOUT CURVE, one tick behind the touch, $ per contract")
